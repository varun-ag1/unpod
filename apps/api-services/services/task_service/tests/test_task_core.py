...
def test_process_task(service):
    # Test data
    task_id = "T123"

    # Mock find_one and create methods
    with patch.object(TaskModel, "find_one") as mock_find_one, patch.object(
        TaskExecutionLogModel, "create"
    ) as mock_create_log, patch.object(
        TaskModel, "update_one"
    ) as mock_update_task, patch.object(
        TaskExecutionLogModel, "update_one"
    ) as mock_update_log:

        mock_task = Mock()
        mock_task.task_id = task_id
        mock_task.run_id = "R123"
        mock_task.input = {}
        mock_task.space_id = "space1"
        mock_find_one.return_value = mock_task
