...
APP_MACHINE_ID = os.environ.get("APP_MACHINE_ID", None)
...
def lower_16bit_private_ip() -> int:
    ...
    """
    ip: ipaddress.IPv4Address = ipaddress.ip_address("127.0.0.1")
    ip_bytes = ip.packed
...
class SonyFlake:
    ...
    def get_elapsed_time(self, timestamp: int) -> int:
        ...
        return (
            self.to_sonyflake_time(datetime.datetime.fromtimestamp(timestamp, UTC))
            - self.start_time
        )
