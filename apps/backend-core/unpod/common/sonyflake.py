...
# 2013-05-01 00:00:00 +0000 UTC
APP_SONYFLAKE_EPOCH = datetime.datetime(2013, 5, 1, 0, 0, 0, tzinfo=UTC)

BIT_LEN_TIME = 39
BIT_LEN_SEQUENCE = 8
BIT_LEN_MACHINE_ID = 63 - (BIT_LEN_TIME + BIT_LEN_SEQUENCE)
SONYFLAKE_EPOCH = APP_SONYFLAKE_EPOCH

def get_machine_id() -> Union[int, None]:
    machine_id = os.environ.get("APP_MACHINE_ID", 257)
    return machine_id
...
def lower_16bit_private_ip() -> int:
    ...
    """
    ip: ipaddress.IPv4Address = ipaddress.ip_address(
...
class SonyFlake:
    ...
    def decompose(_id: int) -> Dict[str, int]:
        ...
        timestamp = int(SONYFLAKE_EPOCH.timestamp() + int(time / 100))
        return {
            "id": _id,
            "msb": msb,
            "time": time,
...
APP_MACHINE_ID = get_machine_id
app_sonyflake = SonyFlake(machine_id=APP_MACHINE_ID)
