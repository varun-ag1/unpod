...
from super.core.utils.text_processing import make_url_compatible
...
class BasicOwnerInfo(BaseModel):
    ...
    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, BasicOwnerInfo):
            return False
        return (
            self.display_name,
            self.first_name,
...
class DocumentBase(BaseModel):
    """Used for Unpod ingestion api, the ID is inferred before use if not provided"""
    ...
    # Owner, creator, etc.
    primary_owners: list[BasicOwnerInfo] | None = None
    # Assignee, space owner, etc.
    secondary_owners: list[BasicOwnerInfo] | None = None
...
class Document(DocumentBase, BaseDocument):
    ...
    source: str | None = None
    object_content: Optional[str] = None
    ...
    def to_dict(self):
        return {
            ...
            "title": self.title,
            "from_ingestion_api": self.from_ingestion_api,
        }
