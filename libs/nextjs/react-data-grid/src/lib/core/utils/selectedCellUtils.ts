...
export function isCellEditableUtil<R, SR>(
...
type GetNextSelectedCellPositionOpts<R, SR> = {
  moveUp: boolean;
  moveNext: boolean;
...
> & {
  ...
  if (
    ...
    return getColSpan(column, lastLeftFixedColumnIndex, {
      type: 'SUMMARY',
 ...
 * @param idx2
 */
export const getMinMaxIdx = (
  idx: number,
  idx2: number | undefined,
): [number, number] => {
  return idx2 === undefined
    ? [idx, idx]
    : [Math.min(idx, idx2), Math.max(idx, idx2)];
};
