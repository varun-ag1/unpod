...
function TreeDataGrid<R, SR, K extends Key>(
  ...
  const { columns, groupBy } = useMemo(() => {
    ...
    for (const [index, column] of columns.entries()) {
      if (rawGroupBy.includes(column.dataIndex)) {
        groupBy.push(column.dataIndex);
        columns[index] = {
          ...column,
          frozen: true,
          fixed: 'left',
    ...
    (row: R | GroupRow<R>) => {
      ...
      for (let i = rowIdx - 1; i >= 0; i--) {
        ...
        if (
          ...
          (!isGroupRow(row) || row.parentId === parentRow.id)
        ) {
  ...
  const rowKeyGetter = useCallback(
    (row: R | GroupRow<R>) => {
      ...
      if (typeof rawRowKeyGetter === 'function') {
        return rawRowKeyGetter(row, rowKey);
      }
    ...
    for (const row of rows) {
      if (isGroupRow(row)) {
        ...
        const isGroupRowSelected = row.childRows.every((cr) =>
          rawSelectedRows.has(rawRowKeyGetter(cr, rowKey)),
        );
      ...
      if (selectedRows?.has(key) && !newSelectedRows.has(key)) {
        if (isGroupRow(row)) {
          ...
          for (const cr of row.childRows) {
            newRawSelectedRows.delete(rawRowKeyGetter(cr, rowKey));
          }
        } else {
        ...
        if (isGroupRow(row)) {
          ...
          for (const cr of row.childRows) {
            newRawSelectedRows.add(rawRowKeyGetter(cr, rowKey));
          }
        } else {
...
export default forwardRef(TreeDataGrid) as <
  R,
  SR = unknown,
  K extends Key = Key,
>(
