...
const CreateNote = ({
  ...
  useEffect(() => {
    if (dataFetched && !isEdit) {
      debounceFn(`create-note-${postSlug || spaceSlug}`, {
        title: title,
        content: content,
        privacy: privacyType,
        users: userList,
  ...
  useEffect(() => {
    if (!dataFetched) {
      if (!isEdit) {
        const draftData = getDraftData(`create-note-${postSlug || spaceSlug}`);
  ...
  const savePodcast = (payload) => {
    if (currentPost?.slug && isEdit) {
      ...
      postDataApi(
        ...
        .then((response) => {
          localStorage.removeItem(`create-note-${postSlug || spaceSlug}`);
          infoViewActionsContext.showMessage(response.message);
          setCreatingPost(false);
          if (onSaved) onSaved(response);
        })
